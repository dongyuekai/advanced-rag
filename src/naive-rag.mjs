...
async function retrieveRelevantDocument(question, k = TOP_K) {
  try {
    // 检索节点就是把query向量化从Milvus里检索相关文档
...
const retrieveNode = async state => {
  // 检索节点就是把query向量化从Milvus里检索相关文档
  const documents = await retrieveRelevantDocument(state.question, state.k);
  return {
...
async function main() {
  ...
  try {
    ...
    console.log(`✓ 集合 ${COLLECTION_NAME} 已加载（之前已加载）\n`);
  }
