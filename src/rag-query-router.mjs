...
const routeQuestionNode = async state => {
  ...
  const route = await router.invoke(`
    你是问答路由器。请判断用户问题是否需要外部检索，并严格以 JSON 返回。
    规则：
     - simple: 常识问答、简短定义、无需特定小说细节即可回答。
     - complex: 需要《天龙八部》具体情节、人物关系、章节事实、原文细节或证据支持。
    用户问题：${state.question}
    输出要求：必须是 json 对象，仅包含 strategy 和 reason 两个字段。
  `)
...
const directAnswerNode = async state => {
  ...
  let generation = ''
  const stream = await llm.stream(`你是一个中文回答助手，请直接简洁回答问题。
    问题：${state.question}
    `)
  ...
  if (result.strategy === "complex") {
    ...
    if (result.documents.length === 0) {
      ...
      result.documents.forEach((item, i) => {
        ...
        console.log(`\n[片段 ${i + 1}] 相似度: ${Number.isFinite(score) ? score.toFixed(4) : String(item.score)}`);
